...
contract ArrakisV2 is IUniswapV3MintCallback, ArrakisV2Storage {
    ...
    function uniswapV3MintCallback(uint256 amount0Owed_, uint256 amount1Owed_, bytes calldata /*_data*/ )
        external
        override
    {
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        if (isTotalSupplyGtZero) {
            ...
            amount0 = FullMath.mulDivRoundingUp(mintAmount_, init0M, denominator);
            amount1 = FullMath.mulDivRoundingUp(mintAmount_, init1M, denominator);
            ...
            uint256 amount0Mint = init0M != 0 ? FullMath.mulDiv(amount0, denominator, init0M) : type(uint256).max;
            uint256 amount1Mint = init1M != 0 ? FullMath.mulDiv(amount1, denominator, init1M) : type(uint256).max;

            require((amount0Mint < amount1Mint ? amount0Mint : amount1Mint) == mintAmount_, "A0&A1");
        }
        ...
        if (isTotalSupplyGtZero) {
            for (uint256 i; i < _ranges.length; i++) {
                ...
                IUniswapV3Pool pool = IUniswapV3Pool(factory.getPool(address(token0), address(token1), range.feeTier));
                uint128 liquidity = Position.getLiquidityByRange(pool, me, range.lowerTick, range.upperTick);

                liquidity = SafeCast.toUint128(FullMath.mulDiv(liquidity, mintAmount_, ts));
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        for (uint256 i; i < _ranges.length; i++) {
            Range memory range = _ranges[i];
            IUniswapV3Pool pool = IUniswapV3Pool(factory.getPool(address(token0), address(token1), range.feeTier));
            uint128 liquidity = Position.getLiquidityByRange(pool, address(this), range.lowerTick, range.upperTick);

            liquidity = SafeCast.toUint128(FullMath.mulDiv(liquidity, burnAmount_, ts));

            if (liquidity == 0) continue;

            Withdraw memory withdraw = _withdraw(pool, range.lowerTick, range.upperTick, liquidity);
        ...
        uint256 leftOver0 = token0.balanceOf(address(this)) - managerBalance0 - total.burn0;
        uint256 leftOver1 = token1.balanceOf(address(this)) - managerBalance1 - total.burn1;
    ...
    // solhint-disable-next-line function-max-lines, code-complexity
    function rebalance(Rebalance calldata rebalanceParams_) external onlyManager nonReentrant {
        // Burns.
...

        {
            ...
            for (uint256 i; i < rebalanceParams_.burns.length; i++) {
                IUniswapV3Pool pool = IUniswapV3Pool(
                    mFactory.getPool(address(mToken0), address(mToken1), rebalanceParams_.burns[i].range.feeTier)
                );
                ...
                if (rebalanceParams_.burns[i].liquidity == type(uint128).max) {
                    ...
                    liquidityToWithdraw = rebalanceParams_.burns[i].liquidity;
                }
                ...
                if (liquidityToWithdraw == liquidity) {
                    (bool exists, uint256 index) = Position.rangeExists(_ranges, rebalanceParams_.burns[i].range);
                    require(exists, "RRNE");
        ...
        if (rebalanceParams_.swap.amountIn > 0) {
            ...
            mToken0.safeApprove(address(rebalanceParams_.swap.router), balance0Before);
            mToken1.safeApprove(address(rebalanceParams_.swap.router), balance1Before);

            (bool success,) = rebalanceParams_.swap.router.call(rebalanceParams_.swap.payload);
            require(success, "SC");
            ...
            if (rebalanceParams_.swap.zeroForOne) {
                require(
                    (balance1After >= balance1Before + rebalanceParams_.swap.expectedMinReturn)
                        && (balance0After >= balance0Before - rebalanceParams_.swap.amountIn),
                    "SF"
                );
                ...
                require(
                    (balance0After >= balance0Before + rebalanceParams_.swap.expectedMinReturn)
                        && (balance1After >= balance1Before - rebalanceParams_.swap.amountIn),
                    "SF"
                );
        ...
        for (uint256 i; i < rebalanceParams_.mints.length; i++) {
            (bool exists,) = Position.rangeExists(_ranges, rebalanceParams_.mints[i].range);
            address pool = factory.getPool(address(token0), address(token1), rebalanceParams_.mints[i].range.feeTier);
            if (!exists) {
                // check that the pool exists on Uniswap V3.

                require(pool != address(0), "NUP");
                require(_pools.contains(pool), "P");
                require(Pool.validateTickSpacing(pool, rebalanceParams_.mints[i].range), "RTS");
    ...
    function _withdraw(IUniswapV3Pool pool_, int24 lowerTick_, int24 upperTick_, uint128 liquidity_)
        internal
        returns (Withdraw memory withdraw)
    {
        (withdraw.burn0, withdraw.burn1) = pool_.burn(lowerTick_, upperTick_, liquidity_);

        (uint256 collect0, uint256 collect1) = _collectFees(pool_, lowerTick_, upperTick_);
