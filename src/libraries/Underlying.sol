...
import {UnderlyingPayload, RangeData, PositionUnderlying, ComputeFeesPayload, GetFeesPayload} from "../structs/SArrakisV2.sol";
...
library Underlying {
    ...
    function totalUnderlyingForMint(
        ...
        for (uint256 i; i < underlyingPayload_.ranges.length; i++) {
            {
                ...
                (uint256 a0, uint256 a1, uint256 f0, uint256 f1) = underlyingMint(
                    RangeData({self: underlyingPayload_.self, range: underlyingPayload_.ranges[i], pool: pool}),
                    mintAmount_,
                    totalSupply_
                );
                amount0 += a0;
        ...
        (uint256 fee0After, uint256 fee1After) = subtractAdminFees(fee0, fee1, arrakisV2.managerFeeBPS());

        amount0 += FullMath.mulDivRoundingUp(
            mintAmount_,
            fee0After +
                IERC20(underlyingPayload_.token0).balanceOf(underlyingPayload_.self) -
        ...
        amount1 += FullMath.mulDivRoundingUp(
            mintAmount_,
            fee1After +
                IERC20(underlyingPayload_.token1).balanceOf(underlyingPayload_.self) -
    ...
    function totalUnderlyingWithFees(
        UnderlyingPayload memory underlyingPayload_
    ) public view returns (uint256 amount0, uint256 amount1, uint256 fee0, uint256 fee1) {
    ...
    function underlying(
        ...
        (amount0, amount1, fee0, fee1) = getUnderlyingBalances(positionUnderlying);
    }
    ...
    function underlyingMint(
        ...
        (amount0, amount1, fee0, fee1) = getUnderlyingBalancesMint(positionUnderlying, mintAmount_, totalSupply_);
    }
    ...
    function getUnderlyingBalancesMint(
        ...
        uint256 mintAmount_,
        uint256 totalSupply_
    ) public view returns (uint256 amount0Current, uint256 amount1Current, uint256 fee0, uint256 fee1) {
        uint128 liquidity;
        {
            ...
            uint128 tokensOwed0;
            uint128 tokensOwed1;
            (liquidity, feeGrowthInside0Last, feeGrowthInside1Last, tokensOwed0, tokensOwed1) = positionUnderlying_
                .pool
                .positions(positionUnderlying_.positionId);
        ...
        (amount0Current, amount1Current) = getAmountsForDelta(
            ...
            SafeCast.toInt128(SafeCast.toInt256(FullMath.mulDiv(uint256(liquidity), mintAmount_, totalSupply_)))
        );
    }
    ...
    function getAmountsForDelta(
        ...
        if (sqrtRatioX96 < sqrtRatioAX96) {
            amount0 = SafeCast.toUint256(SqrtPriceMath.getAmount0Delta(sqrtRatioAX96, sqrtRatioBX96, liquidity));
        } else if (sqrtRatioX96 < sqrtRatioBX96) {
            amount0 = SafeCast.toUint256(SqrtPriceMath.getAmount0Delta(sqrtRatioX96, sqrtRatioBX96, liquidity));
            amount1 = SafeCast.toUint256(SqrtPriceMath.getAmount1Delta(sqrtRatioAX96, sqrtRatioX96, liquidity));
        } else {
            amount1 = SafeCast.toUint256(SqrtPriceMath.getAmount1Delta(sqrtRatioAX96, sqrtRatioBX96, liquidity));
        }
    }
    ...
    function computeMintAmounts(
        ...
        if (current0_ == 0 && current1_ > 0) {
            ...
            uint256 amount0Mint = FullMath.mulDiv(amount0Max_, totalSupply_, current0_);
            uint256 amount1Mint = FullMath.mulDiv(amount1Max_, totalSupply_, current1_);
    ...
    // solhint-disable-next-line function-max-lines
    function _getFeesEarned(GetFeesPayload memory feeInfo_) private view returns (uint256 fee0, uint256 fee1) {
        (, , uint256 feeGrowthOutside0Lower, uint256 feeGrowthOutside1Lower, , , , ) = feeInfo_.pool.ticks(
            feeInfo_.lowerTick
        );
        (, , uint256 feeGrowthOutside0Upper, uint256 feeGrowthOutside1Upper, , , , ) = feeInfo_.pool.ticks(
            feeInfo_.upperTick
        );
    ...
    function _totalUnderlyingWithFees(
        UnderlyingPayload memory underlyingPayload_,
        uint160 sqrtPriceX96_
    ) private view returns (uint256 amount0, uint256 amount1, uint256 fee0, uint256 fee1) {
        for (uint256 i; i < underlyingPayload_.ranges.length; i++) {
            {
                IUniswapV3Pool pool = IUniswapV3Pool(
                    underlyingPayload_.factory.getPool(
    ...
    function _computeFeesEarned(ComputeFeesPayload memory computeFees_) private pure returns (uint256 fee) {
        unchecked {
            ...
            if (computeFees_.tick >= computeFees_.lowerTick) {
                ...
                feeGrowthBelow = computeFees_.feeGrowthGlobal - computeFees_.feeGrowthOutsideLower;
            }
            ...
            if (computeFees_.tick < computeFees_.upperTick) {
                ...
                feeGrowthAbove = computeFees_.feeGrowthGlobal - computeFees_.feeGrowthOutsideUpper;
            }

            uint256 feeGrowthInside = computeFees_.feeGrowthGlobal - feeGrowthBelow - feeGrowthAbove;
            fee = FullMath.mulDiv(
