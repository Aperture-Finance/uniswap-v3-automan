...
abstract contract ArrakisV2Storage is OwnableUpgradeable, ERC20Upgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(string calldata name_, string calldata symbol_, InitializePayload calldata params_)
        external
        initializer
    {
    ...
    function setManagerFeeBPS(uint16 managerFeeBPS_) external onlyManager nonReentrant {
    ...
    function _withdrawManagerBalance() internal {
        ...
        if (amount0 > 0) {
            ...
            try token0.transfer(manager, amount0) {}
            catch {
                amount0 = 0;
            }
        }
        ...
        if (amount1 > 0) {
            // solhint-disable-next-line no-empty-blocks
            try token1.transfer(manager, amount1) {}
            catch {
                amount1 = 0;
            }
        }
    ...
    function _collectFeesOnPools() internal {
        ...
        for (uint256 i; i < _ranges.length; i++) {
            ...
            (uint256 collect0, uint256 collect1) = _collectFees(pool, range.lowerTick, range.upperTick);
    ...
    function _collectFees(IUniswapV3Pool pool_, int24 lowerTick_, int24 upperTick_)
        internal
        returns (uint256 collect0, uint256 collect1)
    {
        (collect0, collect1) =
            pool_.collect(address(this), lowerTick_, upperTick_, type(uint128).max, type(uint128).max);
    }

    function _whitelistRouters(address[] calldata routers_) internal {
        for (uint256 i = 0; i < routers_.length; i++) {
            require(routers_[i] != address(token0) && routers_[i] != address(token1), "RT");
