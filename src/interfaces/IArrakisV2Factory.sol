...
interface IArrakisV2Factory {
    ...
    function deployVault(InitializePayload calldata params_, bool isBeacon_) external returns (address vault);

    function upgradeVaults(address[] memory vaults_) external;

    function upgradeVaultsAndCall(address[] memory vaults_, bytes[] calldata datas_) external;
