...
import {PositionLiquidity, UnderlyingPayload, UnderlyingOutput, Range} from "../structs/SArrakisV2.sol";
...
interface IArrakisV2Helper {
    ...
    function totalUnderlying(IArrakisV2 vault_) external view returns (uint256 amount0, uint256 amount1);
    ...
    function totalLiquidity(IArrakisV2 vault_) external view returns (PositionLiquidity[] memory liquidities);

    function token0AndToken1ByRange(Range[] calldata ranges_, address token0_, address token1_, address vaultV2_)
        external
        view
    ...
    function token0AndToken1PlusFeesByRange(
        ...
        external
        view
        returns (Amount[] memory amount0s, Amount[] memory amount1s, Amount[] memory fee0s, Amount[] memory fee1s);
}
